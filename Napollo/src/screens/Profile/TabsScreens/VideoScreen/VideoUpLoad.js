...
import FollowUser from '../../component/FollowActivitiesComp/FollowUser'
...
const Activities = () => {
  ...
  const followerArray = data.filter(item => item.activityType === 'FOLLOW');
  ...
  return (
    <View style={styles.container}>
      ...
      {followerArray.slice(0, 2).map((item, index) => (
        <FollowUser {...item} key={index} />
      ))}
